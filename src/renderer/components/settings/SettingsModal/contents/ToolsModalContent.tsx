...
const SpeechInputModeSection: React.FC<{
  mode: SpeechInputMode;
  onChange: (mode: SpeechInputMode) => void;
}> = ({ mode, onChange }) => {
  ...
  return (
    <div className='px-[12px] md:px-[32px] py-[24px] bg-2 rd-12px md:rd-16px border border-border-2'>
      <div className='flex flex-col gap-4px mb-16px'>
        <span className='text-14px text-t-primary'>{t('settings.speechInputMode')}</span>
        <span className='text-13px text-t-secondary'>{t('settings.speechInputModeDescription')}</span>
      </div>

      <Form layout='horizontal' labelAlign='left'>
        <Form.Item label={t('settings.speechInputMode')}>
          <AionSelect value={mode} onChange={(value) => onChange(value as SpeechInputMode)}>
            <AionSelect.Option value='local'>{t('settings.speechInputModeLocal')}</AionSelect.Option>
            <AionSelect.Option value='remote'>{t('settings.speechInputModeRemote')}</AionSelect.Option>
          </AionSelect>
        </Form.Item>
      </Form>
      ...
      <div className='mt-8px text-13px text-t-secondary'>
        {mode === 'local'
          ? t('settings.speechInputModeLocalDescription')
          : t('settings.speechInputModeRemoteDescription')}
      </div>
    </div>
  );
};
...
const ToolsModalContent: React.FC = () => {
  ...
  >();
  const [speechInputMode, setSpeechInputMode] = useState<SpeechInputMode>(DEFAULT_SPEECH_INPUT_MODE);
  ...
  useEffect(() => {
    const loadConfigs = async () => {
      try {
        ...
        const storedSpeechInputMode = await ConfigStorage.get('tools.speechInputMode');
        const storedSpeechToTextConfig = await ConfigStorage.get('tools.speechToText');
        if (storedModel) {
          setImageGenerationModel(storedModel);
        }
        setSpeechInputMode(normalizeSpeechInputMode(storedSpeechInputMode));
  ...
  const updateSpeechInputMode = useCallback((mode: SpeechInputMode) => {
    const nextMode = normalizeSpeechInputMode(mode);
    setSpeechInputMode(nextMode);
    ConfigStorage.set('tools.speechInputMode', nextMode).catch((error) => {
      console.error('Failed to save speech input mode:', error);
    });
    ...
    <div className='flex flex-col h-full w-full'>
      ...
      <AionScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
        <div className='space-y-16px'>
          ...
          <SpeechInputModeSection mode={speechInputMode} onChange={updateSpeechInputMode} />
